...
def lore_html():
    ...
    _lore_html_cache = (
        ...
        '<div id="toc"><div class="toc-top"><span>📑 目录</span><button onclick="hideToc()" title="隐藏目录">«</button></div><div id="tocbody">' + "".join(toc) + "</div></div>"
        + '<div id="content"><button id="toc-show" onclick="showToc()" style="display:none">☰ 显示目录</button>' + "".join(parts) + "</div>"
        + "<script>function hideToc(){document.getElementById('toc').style.display='none';"
        + "document.getElementById('content').style.marginLeft='0';"
        + "document.getElementById('toc-show').style.display='inline-block';}"
        + "function showToc(){document.getElementById('toc').style.display='block';"
        + "document.getElementById('content').style.marginLeft='260px';"
        + "document.getElementById('toc-show').style.display='none';}</script>"
    )
    return _lore_html_cache
