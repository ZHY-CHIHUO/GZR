...
### 检索模型实测对比（2026-08-15，30 题测试集、只计正文命中、全库检索）

| 模型 | rrf@3 | rrf@5 | rrf@8 | 查询耗时 | 说明 |
|---|---|---|---|---|---|
| **bge-small-zh-v1.5**（默认） | 27% | 37% | 37% | 0.019s | 最快、体积最小（90MB），日常够用 |
| **jina-v2-base-zh**（推荐更准） | 40% | 43% | 43% | 0.034s | 中文专用，rrf@3 高 13pp、rrf@5 高 6pp |
| BGE-M3(int8) | — | 37%* | 43%* | — | *候选池对比值；int8 量化 + CPU 全量建库不可行，未达预期，不推荐 |

- 参数结论：`k=5` 已足够（k=8 无增益）；RRF 混合检索比纯向量高 10~20pp；摘要增强对剧情类问题 top-1 命中 2/8 → 7/8
